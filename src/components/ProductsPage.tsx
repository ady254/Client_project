    ...
    <div className="min-h-screen bg-slate-900 pt-32 pb-20 px-6">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-16">
          {categories.map((category) => (
            <button
              ...
              className={`px-8 py-4 rounded-full text-sm tracking-wider transition-all duration-300 ${
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/95 backdrop-blur-xl">
          <div className="relative max-w-4xl w-full bg-slate-800/50 backdrop-blur-sm border border-white/10 rounded-3xl overflow-hidden">
            <button
              onClick={() => setSelectedProduct(null)}
              className="absolute top-6 right-6 z-10 p-2 bg-white/10 rounded-full text-white hover:bg-white/20 transition-all duration-300"
            >
              <X size={24} />
            </button>
